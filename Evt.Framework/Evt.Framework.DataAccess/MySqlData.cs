...
using Evt.Framework.PerformanceMonitor;
...
namespace Evt.Framework.DataAccess
{
    ...
    /// 封装SqlServer的访问方法
    /// </summary>
    public class MySqlData : AbstractData, IData, IDisposable
    {
        #region Properties
        ...
        public MySqlData(string connectionString, bool impersonated)
        {
        ...
        public void OpenConnection()
        {
            ...
            if (this._impersonated)
            {
                ...
                {
                    cnn = new MySqlConnection(this._connectionString);
        ...
        public int ExecuteNonQuery(string sql, CommandTypeEnum type, ParameterCollection parameters)
        {
            ...
            TraceUtil.TraceDebug(sql + (parameters == null ? "" : parameters.ToString()));
        ...
        public DataTable ExecuteDataTable(string sql, CommandTypeEnum type, ParameterCollection parameters, int pageIndex, int pageSize)
        {
            if (type == CommandTypeEnum.Text)
            {
                return GetDataTableByLimit(sql, type, parameters, pageIndex, pageSize);
            }
            else
            {
        ...
        // 用Limit方式
        private DataTable GetDataTableByLimit(string sql, CommandTypeEnum type, ParameterCollection parameters, int pageIndex, int pageSize)
        {
            int startIndex = pageSize * (pageIndex - 1);

            string sqlCommand = sql + " LIMIT " + startIndex.ToString() + "," + pageSize.ToString();

            TraceUtil.TraceDebug(sqlCommand + (parameters == null ? "" : parameters.ToString()));

            return ExecuteDataTable(sqlCommand, CommandTypeEnum.Text, parameters);
        }
        ...
        private DataTable GetDataTableByDataReader(string sql, CommandTypeEnum type, ParameterCollection parameters, int pageIndex, int pageSize)
        {
            ...
            try
            {
                ...
                // 执行
                MySqlDataReader dr = oleCmd.ExecuteReader();
        ...
        public DataSet ExecuteDataSet(string sql, CommandTypeEnum type, ParameterCollection parameters, int pageIndex, int pageSize)
        {
            if (type == CommandTypeEnum.Text)
            {
                return GetDataSetByLimit(sql, type, parameters, pageIndex, pageSize);
            }
            else
            {
                return GetDataSetByDataReader(sql, type, parameters, pageIndex, pageSize);
            }
        }

        // 用Limit方式
        private DataSet GetDataSetByLimit(string sql, CommandTypeEnum type, ParameterCollection parameters, int pageIndex, int pageSize)
        {
            int startIndex = pageSize * (pageIndex - 1);
            ...
            return ExecuteDataSet(sqlCommand, CommandTypeEnum.Text, parameters);
        }

        // 用Reader方式
        [PerformanceMonitor]
        private DataSet GetDataSetByDataReader(string sql, CommandTypeEnum type, ParameterCollection parameters, int pageIndex, int pageSize)
        {
            Interlocked.Increment(ref count);
