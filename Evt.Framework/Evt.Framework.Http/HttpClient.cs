...
namespace Evt.Framework.Http
{
    public class HttpClient
    {
        ...
        public void AttachFile(string fileName, string fieldName, bool isGzipCompress = true)
        {
            HttpFile file = new HttpFile(fileName, fieldName, isGzipCompress);
            files.Add(file);

            if (isGzipCompress)
            {
                postingData.Add("isgzip", "1");
            }
            else
            {
                postingData.Add("isgzip", "0");
            }
        }
        ...
        /// <param name="fieldName">文件字段的名称(相当于&lt;input type=file name=fieldName&gt;)里的fieldName)</param>
        public void AttachFile(byte[] data, string fileName, string fieldName, bool isGzipCompress = true)
        {
            HttpFile file = new HttpFile(data, fileName, fieldName, isGzipCompress);
            files.Add(file);
        ...
        private HttpWebRequest CreateRequest()
        {
            ...
            {
                throw new HttpClientException(ex.Message, "执行CreateRequest失败", ex);
            }
        ...
        public byte[] GetBytes()
        {
            ...
            {
                throw new HttpClientException(ex.Message, "执行GetBytes失败", ex);
            }
